...
class Weixinpay {
    ...
    private $config=array(
        'APPID'=>'wxf8e92540552c94c3',// 微信支付APPID
            'MCHID'=>'1488223862',// 微信支付MCHID 商户收款账号
            'KEY'=>'1488223862Mintca1488223862Mintca', // 微信支付KEY
            'APPSECRET'=>'b3f417a7d759e5320d34292989b9b538', //公众帐号secert
    );
...
//https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx2428d1311fc459d8&redirect_uri=http%3A%2F%2FRexmix%2Fmintcat%2Findex.php%2FAdmin%2FWeixinpay%2Fpay&response_type=code&scope=snsapi_base&state=1#wechat_redirect！
    /**
     * 获取jssdk需要用到的数据
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url
            $redirect_uri=U('Admin/Weixinpay/pay','','',true);
            ...
            $map['tradeid'] = $out_trade_no;
            $find = M('order') ->where($map) ->find();
            if(!empty($find)){
                $money = $find['tradetotal'];
                $body = $find['car_no'];
                $id = $find['id'];
            ...
            $order=array(
                ...
                'product_id'=>$id,// 商品id（需要根据自己的业务修改）
