...
class WxApi
{
     ...
     */
     function __construct($app_id,$app_secret)
     {
          ...
          $this->oauth = new WxAuthV2($this->access_token);
     }
  ...
  /**
      * 删除菜单 API：http://mp.weixin.qq.com/wiki/index.php?title=%E8%87%AA%E5%AE%9A%E4%B9%89%E8%8F%9C%E5%8D%95%E5%88%A0%E9%99%A4%E6%8E%A5%E5%8F%A3
      * 
     */
     function menu_delete()
     {
          $params = array();
          $data=$this->oauth->get('menu/delete', $params);
		  if ($data['errcode']==0) {  
           return true;  
			}else{  
            return false;  
 ...
 /**
      * 获取菜单 API：http://mp.weixin.qq.com/wiki/index.php?title=%E8%87%AA%E5%AE%9A%E4%B9%89%E8%8F%9C%E5%8D%95%E5%88%9B%E5%BB%BA%E6%8E%A5%E5%8F%A3
      * 
     */
	 function menu_get()
     {
	 ...
	 /**
      * 创建菜单 API：http://mp.weixin.qq.com/wiki/index.php?title=%E8%87%AA%E5%AE%9A%E4%B9%89%E8%8F%9C%E5%8D%95%E5%88%9B%E5%BB%BA%E6%8E%A5%E5%8F%A3
      * 
     */
     function menu_create($params)
     {
		  $data=$this->oauth->post('menu/create',$params);
		   if ($data['errcode']==0) {  
				return true;  
			}else{  
            return false;  
	...
	*
	*/
	function media_uploadnews($newsArray){
     	$params = array("");
     	$params['articles']=$newsArray;
     	return $this->oauth->post('media/uploadnews',$params);
     }
	 /*
	*
	*根据OPENID1群发消息
	...
	*
	*/
	 function send_message_openids($touser,$mpnews){
     	$params = array("");
     	$params['touser']=$touser;
     	$params['mpnews']=$mpnews;
     	$params['msgtype']="mpnews";
     	return $this->oauth->post('message/mass/send',$params);
     }

	  function send_template_message($touser,$template_id,$url,$data){
     	$params = array("");
     	$params['touser']=$touser;
     	$params['template_id']=$template_id;
     	$params['url']=$url;
     	$params['data']=$data;
     	$re=$this->oauth->post('message/template/send',$params);
		return $re;
     }
