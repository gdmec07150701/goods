...
var SELF = '/goods/index.php/Seller/Goods/index.html';
var URL = '/goods/index.php/Seller/Goods';
...
<div id="sidebar">
	...
	<ul>
		...
		<?php $arc="xc_w11";if(chkArc($arc)){ ?><li class="submenu <?php if(in_array((CONTROLLER_NAME), explode(',',"Goods"))): ?>active open<?php endif; ?>">
				<a><i class="icon icon-gift active"></i> <span>商品</span></a>
				<ul>
				<?php $arc="xc_w11";if(chkArc($arc)){ ?><li <?php if(in_array((CONTROLLER_NAME), explode(',',"Widthdraw"))): if(in_array((ACTION_NAME), explode(',',"store"))): ?>class="active"<?php endif; endif; ?>><a href="<?php echo U('Goods/index');?>">商品管理</a></li><?php } ?>
				</ul>
			</li><?php } ?>
			...
			<div>
				...
				<tbody>
							 <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="list_data_<?php echo ($vo["id"]); ?>"  class="center">
									...
									<td><?php echo (getManagerRole($vo["role"])); ?></td>
									<td><?php echo ($vo["nickname"]); ?></td>
									<td><?php echo ($vo["mobile"]); ?>&nbsp;&nbsp;/&nbsp;&nbsp;<?php echo ($vo["email"]); ?></td>
									<td><?php echo (getStatus($vo["status"],true)); ?></td>
									<td class="center">									
									<a class="btn btn-info" href="<?php echo U('add?id='.$vo['id']);?>">
										<i class="icon-edit icon-white"></i>  
										编辑
									</a>									
									&nbsp;
									<a class="btn btn-danger" data-toggle="modal" href="#static" onclick="$('#del_mid').val(<?php echo ($vo["id"]); ?>);">
...
function del(){
	...
	$.ajax({
	  type: 'POST',
	  url: "/goods/index.php/Seller/Goods/index.html",
	  data:{id:mid,act:'del'},
	  dataType: "json",
